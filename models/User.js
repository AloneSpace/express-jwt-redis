class User {
    ...
    static Register({username, email, password}) {
        ...
        if(Object(this.findExistUser(username)).hasOwnProperty('username')) return false
        const password_gen = this.generatePassword(password)
        return this.write2JSON([...this.findAll(), {username, email, salt: password_gen.salt, hash: password_gen.hash}])
    }
    ...
    static write2JSON(users = []) {
        const fs = require('fs')
        fs.writeFileSync(`${process.cwd()}/data/users.json`, JSON.stringify(users, null, '\t'))
        return true
    }
